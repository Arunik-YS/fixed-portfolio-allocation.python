...
st.caption("SPYM(주식), TLT(장기채), IAU(금) 조합의 10년 백테스트 기반 리밸런싱")

# 1. 황금 비중 안내
st.subheader("🎯 10년 최적화 타겟 비중")
col_w1, col_w2, col_w3 = st.columns(3)
col_w1.metric("SPYM (S&P 500)", f"{optimal_weights['SPYM']*100:.1f}%")
...
with st.expander("📊 과거 10년 시뮬레이션 및 백테스트 검증", expanded=False):
    ...
    portfolio_daily_ret = (
        daily_returns['SPYM'] * optimal_weights['SPYM'] +
    ...
    cum_spym = (1 + daily_returns['SPYM']).cumprod() * 10000
    
    # 차트용 데이터프레임 통합
    chart_data = pd.DataFrame({
        "황금비중 포트폴리오": cum_portfolio,
        "S&P 500 (주식 올인)": cum_spym
    ...
    mdd_spym = ((cum_spym / cum_spym.cummax()) - 1).min() * 100
    
    stats_df = pd.DataFrame({
        "지표": ["예상 연수익률", "연 변동성(위험)", "샤프 지수(가성비)", "최대 낙폭(MDD)"],
        "황금비중 포트폴리오": [f"{ret_p*100:.2f}%", f"{std_p*100:.2f}%", f"{sharpe_p:.2f}", f"{mdd_portfolio:.1f}%"],
        "S&P 500 올인": [f"{(mean_returns['SPYM']*252)*100:.2f}%", f"{(np.sqrt(cov_matrix.loc['SPYM','SPYM']*252))*100:.2f}%", f"{(mean_returns['SPYM']*252 - RISK_FREE_RATE)/(np.sqrt(cov_matrix.loc['SPYM','SPYM']*252)):.2f}", f"{mdd_spym:.1f}%"]
    })
